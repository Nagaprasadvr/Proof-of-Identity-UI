            ...
            try {
                if (wallet?.publicKey) {
                    const [digitalPdaAcc, bump] = PublicKey.findProgramAddressSync(
                        [Buffer.Buffer.from('dig_identity'), wallet?.publicKey.toBuffer()],
                        sdk.PROGRAM_ID
                    );
    ...
    return (
        <Box>
            ...
            <Box style={{ marginTop: '20px', display: 'flex', justifyContent: 'center' }} className="w3-animate-bottom">
                <input
                    type="text"
                    style={{ color: 'black', height: '50px', width: '600px', borderRadius: '0.5em', fontSize: '20px' }}
            ...
            {open && digIdentityAcc && (
                <ViewIdentityModal
                    open={open}
                    data={digIdentityAcc}
                    digitalIdentityPda={digitalIdentityPda}
