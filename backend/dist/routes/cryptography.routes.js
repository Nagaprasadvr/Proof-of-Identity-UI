...
router.route("/decryptData").post((req, res) => {
  ...
  if (ticker === "solData") {
    ...
    const keypair = fs_1.default.readFileSync(keypairpath);
    const keypairObj = JSON.parse(keypair.toString());
    const decryptedName = (0, RSA_1.decryptData)(
      encData.name,
      keypairObj.privateKey
    );
    const decryptedContactNumber = (0, RSA_1.decryptData)(
      encData.contactNumber,
      keypairObj.privateKey
    );
    const decryptedDob = (0, RSA_1.decryptData)(
      encData.dob,
      keypairObj.privateKey
    );
    const decryptedAddress = (0, RSA_1.decryptData)(
      encData.residenceAddress,
      keypairObj.privateKey
    );
    const decryptedPanNo = (0, RSA_1.decryptData)(
      encData.panNumber,
      keypairObj.privateKey
    );
    const decryptedPassportID = (0, RSA_1.decryptData)(
      encData.passportId,
      keypairObj.privateKey
    );
    const decryptedAadharNumber = (0, RSA_1.decryptData)(
    ...
    const keypair = fs_1.default.readFileSync(keypairpath);
    const keypairObj = JSON.parse(keypair.toString());
    const decryptedpicLink = (0, RSA_1.encryptData)(
      encData.picUploadLink,
      keypairObj.privateKey
    );
    const decryptedaadharLink = (0, RSA_1.encryptData)(
      encData.aadharUploadLink,
      keypairObj.privateKey
    );
    const decryptedpanLink = (0, RSA_1.encryptData)(
      encData.panUploadlink,
      keypairObj.privateKey
    );
    const decryptedpassportLink = (0, RSA_1.encryptData)(
      encData.passportUploadLink,
      keypairObj.privateKey
    );
...
router.route("/getRSAKeypair").get((req, res) => {
  ...
  if (
    fs_1.default.existsSync(keypair_512_path) &&
    fs_1.default.existsSync(keypair_1028_path)
  ) {
